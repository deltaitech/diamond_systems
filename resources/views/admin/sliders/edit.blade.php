...
@section('title', settings()->website_title . ' | ' . __('words.edit_slider'))
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        <!--begin::Breadcrumb-->
        <h5 class="text-dark font-weight-bold my-1 mr-5">{{ __('words.sliders') }}</h5>
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            <li class="breadcrumb-item">
                <a href="{{ route('admin.home') }}" class="text-muted">{{ __('words.home') }}</a>
            </li>
            <li class="breadcrumb-item">
                <a href="{{ route('sliders.index') }}" class="text-muted">{{ __('words.show_sliders') }}</a>
            </li>
            <li class="breadcrumb-item">
                <span class="text-muted">{{ __('words.edit_slider') }}</span>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
    ...
    <div class="card card-custom mb-2">
        <div class="card-header card-header-tabs-line">
            <div class="card-title">
                <h3 class="card-label">{{ __('words.edit_slider') }}</h3>
            </div>
                    ...
                    <div class="tab-pane fade show @if ($key == 0) active @endif"
                        ...
                        <div class="col form-group">
                            ...
                            <textarea class="form-control ckeditor @error($locale . '.description') is-invalid @enderror " type="text"
                                name="{{ $locale . '[description]' }}" rows="4">{{ old($locale . '.description', $slider->translate($locale)->description) }} </textarea>
